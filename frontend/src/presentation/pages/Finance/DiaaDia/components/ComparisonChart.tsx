...
export const ComparisonChart = ({ transactions }: ComparisonChartProps) => {
  // Sumamos ingresos normales y lo que recuperamos de huchas
  const incomes = transactions.filter(t => t.type === 'income' || t.type === 'savings_return').reduce((acc, t) => acc + t.amount, 0);
  
  // Sumamos TODO el dinero que sale (Gastos, Imprevistos, Ahorro e Inversión)
  const expenses = transactions.filter(t => t.type === 'expense' || t.type === 'other_expense' || t.type === 'transfer').reduce((acc, t) => acc + t.amount, 0);
  ...
  const rawData = [
    ...
    { name: 'Salidas', value: expenses, color: '#f87171' }   // Rojo claro
  ];
  ...
  return (
    <div className="bg-[#1a1a1a] border border-[#2d2d2d] rounded-xl p-6 min-h-[350px] flex flex-col shadow-sm">
      <h2 className="font-bold text-gray-200 mb-4">Ingresos vs Salidas</h2>
        ...
        <>
          ...
          <div className="flex justify-center gap-6 mt-4">
            ...
            <div className="flex items-center gap-1.5">
              ...
              <span className="text-xs text-gray-400">Salidas: {expenses.toFixed(2)}€</span>
            </div>
          </div>
        </>
      ) : (
