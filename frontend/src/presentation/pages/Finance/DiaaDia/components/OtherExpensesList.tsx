...
export const OtherExpensesList = ({ transactions, monthId, monthLabel }: OtherExpensesListProps) => {
  ...
  // Aquí filtras por los gastos que consideres "otros" o todos menos los recurrentes, según tu lógica.
  const expenses = transactions
    .filter(t => t.type === 'expense' && t.category !== 'recurring') // Ajusta este filtro según necesites
        ...
        <div className="overflow-y-auto custom-scrollbar space-y-3 mb-4 pr-2 max-h-[460px]">
          {expenses.length > 0 ? (
            ...
            <div className="py-6 flex justify-center items-center">
              <p className="text-gray-500 italic text-sm">No hay gastos extra registrados.</p>
            </div>
          )}
        </div>
