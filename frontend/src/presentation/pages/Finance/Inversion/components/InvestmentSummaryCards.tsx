      ...
      <div className="bg-[#141416] border border-[#2d2d2d] rounded-3xl p-6">
          ...
          <svg viewBox="0 0 400 100" className="w-full h-full preserve-3d" preserveAspectRatio="none">
            <defs>
              <linearGradient id="gradTotal" x1="0" y1="0" x2="0" y2="1">
                <stop offset="0%" stopColor={isUp ? "#10b981" : "#f59e0b"} stopOpacity="0.2" />
                <stop offset="100%" stopColor={isUp ? "#10b981" : "#f59e0b"} stopOpacity="0" />
              </linearGradient>
            </defs>
        ...
        <div className="grid grid-cols-3 gap-4">
          <button onClick={() => onNavigate('bolsa')} className="bg-[#1c1c1e] hover:bg-[#252528] transition-colors border border-[#2d2d2d] rounded-2xl p-4 text-left cursor-pointer">
            ...
            <p className="text-lg font-black text-amber-500">{valorBolsa.toLocaleString('es-ES', { maximumFractionDigits: 2 })} €</p>
          </button>
          <button onClick={() => onNavigate('proyecto')} className="bg-[#1c1c1e] hover:bg-[#252528] transition-colors border border-[#2d2d2d] rounded-2xl p-4 text-left cursor-pointer">
            <p className="text-xs font-bold text-gray-500 mb-1">Proyectos</p>
            <p className="text-lg font-black text-teal-400">{valorProyectos.toLocaleString('es-ES', { maximumFractionDigits: 2 })} €</p>
          </button>
        ...
        <div className="flex flex-col sm:flex-row items-center gap-8">
          ...
          <div className="relative w-32 h-32 shrink-0">
            ...
            <div className="absolute inset-0 flex flex-col items-center justify-center text-center">
              ...
              <span className="text-sm font-black text-white">{Math.round(carteraTotal)}</span>
            </div>
          </div>
          ...
          <div className="flex-1 w-full space-y-4">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2">
                ...
                <span className="text-sm font-bold text-gray-300">Bolsa</span>
              </div>
              <span className="text-sm font-black text-white">{valorBolsa.toLocaleString('es-ES', { maximumFractionDigits: 2 })} €</span>
            </div>
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2">
                <div className="w-3 h-3 rounded-sm bg-[#2dd4bf]"></div>
                <span className="text-sm font-bold text-gray-300">Proyectos</span>
              </div>
              <span className="text-sm font-black text-white">{valorProyectos.toLocaleString('es-ES', { maximumFractionDigits: 2 })} €</span>
            </div>
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2">
                <div className="w-3 h-3 rounded-sm bg-gray-600"></div>
                <span className="text-sm font-bold text-gray-500">Sin asignar</span>
              </div>
              <span className="text-sm font-black text-white">{disponibleGlobal.toLocaleString('es-ES', { maximumFractionDigits: 2 })} €</span>
            </div>
          </div>
        </div>
      </div>
