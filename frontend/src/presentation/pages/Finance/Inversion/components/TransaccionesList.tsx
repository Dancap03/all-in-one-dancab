          ...
          {transacciones.length === 0 ? (
            ...
            transacciones.map((t) => (
              <div key={t.id} className="flex items-center gap-4 py-4 border-l-2 border-transparent hover:bg-[#1a1a1a] -mx-4 px-4 rounded-lg cursor-pointer transition-colors group">
                ...
                <div className="w-10 h-10 rounded-full flex items-center justify-center text-white text-xs font-bold shrink-0 bg-[#2d2d2d]" style={t.logoColor ? { backgroundColor: t.logoColor } : {}}>
                  {t.logoInitial}
                </div>
