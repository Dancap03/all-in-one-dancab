...
import { ArrowLeft, Plus, Code, RefreshCw, MoreVertical, X, Trash2, Package, Tag, ArrowRight, TrendingUp, Minus } from 'lucide-react';
import { db, auth } from '../../../../../../infrastructure/firebase/config';
import { doc, getDoc, setDoc } from 'firebase/firestore';

// 🚀 NUEVO: Interfaz para el historial individual de cada proyecto
export interface ProjectTx {
  id: string;
  type: 'ingreso' | 'gasto' | 'venta' | 'compra';
  amount: number;
  cost?: number; // Para calcular el beneficio de una venta
  profit?: number; // Beneficio limpio de la operación
  date: string;
}
...
export interface Project {
  ...
  activeInvested: number;     // Dinero bloqueado (Stock activo)
  historicalInvested: number; // Gastos totales / Compras totales
  historicalRevenue: number;  // Ingresos totales / Ventas totales
  createdAt: string;
  transactions?: ProjectTx[]; // 🚀 NUEVO: Lista de movimientos
}
...
export const ProyectoDetails = ({
  ...
  const [selectedProject, setSelectedProject] = useState<Project | null>(null); // 🚀 ESTADO PARA ENTRAR AL PROYECTO
    ...
    const loadProjects = async () => {
      ...
      try {
        ...
        if (docSnap.exists()) {
          ...
          // Parche de compatibilidad por si no tenían la lista de transacciones
  ...
  const handleDeleteProject = (proj: Project) => {
    if(confirm(`¿Estás seguro de eliminar el proyecto "${proj.name}"?\nSi tienes stock o capital activo (${proj.activeInvested} €) se devolverá a tu saldo disponible.`)) {
      if (proj.activeInvested > 0) {
        onEjecutarProyecto('balance', proj.activeInvested); 
      }
  ...
  const handleConfirmTx = () => {
    ...
    if (txModal.type === 'gasto' || txModal.type === 'compra') {
      if (amount > proyectoDisponible) { alert(`Saldo insuficiente. Dispones de ${proyectoDisponible.toLocaleString('es-ES')} €.`); return; }
      ...
      newTx.profit = amount; // En software, el ingreso puro es beneficio directo

    } else if (txModal.type === 'venta') {
      if (cost < 0 || cost > p.activeInvested) { alert(`El coste no puede superar el Stock Activo (${p.activeInvested} €)`); return; }
      ...
      // 🚀 Guardamos la matemática de esta venta concreta
      newTx.cost = cost;
      newTx.profit = amount - cost;
    }

    // Guardar transacción en el historial del proyecto
    if (!updatedProjects[pIndex].transactions) updatedProjects[pIndex].transactions = [];
    updatedProjects[pIndex].transactions!.unshift(newTx); // Añadir al principio

    saveProjects(updatedProjects);
    
    // Si estamos dentro del proyecto, actualizamos la vista
    if (selectedProject?.id === p.id) {
      setSelectedProject(updatedProjects[pIndex]);
    }
  ...
  // ============================================================================
  // VISTA 1: LISTADO GLOBAL DE PROYECTOS
  // ============================================================================
  if (!selectedProject) {
    return (
      <div className="w-full mx-auto pb-12 animate-in fade-in duration-300 relative">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
          <div className="bg-[#141416] border border-[#2d2d2d] rounded-2xl p-4 flex flex-col justify-between h-28">
            ...
            <div><p className={`text-2xl font-black ${totalBeneficio >= 0 ? 'text-emerald-400' : 'text-rose-400'}`}>{totalBeneficio >= 0 ? '+' : ''}{totalBeneficio.toLocaleString('es-ES', { minimumFractionDigits: 0 })} €</p></div>
          </div>
          <div className="bg-[#141416] border border-[#2d2d2d] rounded-2xl p-4 flex flex-col justify-between h-28">
            <p className="text-xs font-bold text-gray-500">Capital invertido</p>
            <div><p className="text-2xl font-black text-white">{totalCapitalHistorico.toLocaleString('es-ES', { minimumFractionDigits: 0 })} €</p></div>
          </div>
          <div className="bg-[#141416] border border-[#2d2d2d] rounded-2xl p-4 flex flex-col justify-between h-28">
            <p className="text-xs font-bold text-gray-500">Ingresos totales</p>
            <div><p className="text-2xl font-black text-teal-400">{totalIngresos.toLocaleString('es-ES', { minimumFractionDigits: 0 })} €</p></div>
          </div>
          <div className="bg-[#141416] border border-[#2d2d2d] rounded-2xl p-4 flex flex-col justify-between h-28">
            <p className="text-xs font-bold text-gray-500">ROI medio</p>
            <div><p className={`text-2xl font-black ${roiMedio >= 0 ? 'text-emerald-400' : 'text-rose-400'}`}>{roiMedio > 0 ? '+' : ''}{roiMedio.toFixed(0)}%</p></div>
          </div>
        </div>
        ...
        {projects.length > 0 ? (
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            {projects.map(proj => {
              ...
              return (
                <div 
                  key={proj.id} 
                  onClick={() => setSelectedProject(proj)} // 🚀 ENTRAR AL DETALLE AL HACER CLIC
                  className="bg-[#141416] border border-[#2d2d2d] rounded-3xl p-5 hover:border-[#3d3d3d] hover:bg-[#1a1a1c] transition-colors group relative cursor-pointer"
                >
                  <button onClick={(e) => { e.stopPropagation(); handleDeleteProject(proj); }} className="absolute top-4 right-4 p-2 bg-red-500/10 text-red-500 hover:bg-red-500 hover:text-white rounded-lg opacity-0 group-hover:opacity-100 transition-opacity cursor-pointer z-10">
                    <Trash2 size={16} />
                  </button>
                  ...
                  <div className="grid grid-cols-3 gap-4 mb-4">
                    <div className="bg-[#1c1c1e] p-2.5 rounded-xl border border-[#2d2d2d]">
                      <p className="text-[10px] font-bold text-gray-500 mb-1 leading-tight">{isSoftware ? 'Invertido' : 'Total Compras'}</p>
                      <p className="text-sm font-black text-rose-400">-{proj.historicalInvested.toLocaleString()} €</p>
                    </div>
                    <div className="bg-[#1c1c1e] p-2.5 rounded-xl border border-[#2d2d2d]">
                      <p className="text-[10px] font-bold text-gray-500 mb-1 leading-tight">{isSoftware ? 'Ingresos' : 'Ventas Totales'}</p>
                      <p className="text-sm font-black text-teal-400">+{proj.historicalRevenue.toLocaleString()} €</p>
                    </div>
                    <div className="bg-[#1c1c1e] p-2.5 rounded-xl border border-[#2d2d2d]">
                      <p className="text-[10px] font-bold text-gray-500 mb-1 leading-tight">{isSoftware ? 'Beneficio' : 'Stock (Coste)'}</p>
                      {isSoftware ? (
                  ...
                  <div className="flex justify-between items-center text-[11px] font-medium text-gray-500 border-t border-[#2d2d2d] pt-3 mt-4">
                    <p className="flex items-center gap-1">Ver libro de cuentas <ArrowRight size={12}/></p>
          ...
          <div className="text-center py-16 bg-[#141416] border border-[#2d2d2d] rounded-3xl">
            ...
            <button onClick={() => setIsNewProjectOpen(true)} className="bg-[#f59e0b] hover:bg-[#ca8a04] text-black px-6 py-3 rounded-xl font-bold transition-colors cursor-pointer mt-4">Crear mi primer proyecto</button>
          </div>
        )}

        {/* MODAL NUEVO PROYECTO AQUÍ ABAJO... */}
        {isNewProjectOpen && (
              ...
              <div className="p-5 space-y-4">
                ...
                <div>
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    <button onClick={() => setNewProjectType('software')} className={`py-3 rounded-xl border text-sm font-bold transition-colors cursor-pointer ${newProjectType === 'software' ? 'bg-amber-500/10 border-amber-500 text-amber-500' : 'bg-[#1c1c1e] border-[#2d2d2d] text-gray-400 hover:text-white'}`}>Software</button>
                ...
                <button onClick={handleCreateProject} className="w-full bg-amber-500 text-black py-3.5 rounded-xl font-black mt-4">Crear Proyecto</button>
              </div>
            </div>
          </div>
        )}
  ...
  // ============================================================================
  // VISTA 2: DETALLE DEL PROYECTO INDIVIDUAL 🚀 (EL LIBRO DE CUENTAS)
  // ============================================================================
  ...
  return (
    <div className="w-full mx-auto pb-12 animate-in slide-in-from-right-8 duration-300 relative">
      ...
      <div className="flex items-center justify-between mb-8">
        <div className="flex items-center gap-4">
          <button onClick={() => setSelectedProject(null)} className="p-2 -ml-2 text-gray-400 hover:text-white transition-colors rounded-lg hover:bg-[#2d2d2d] cursor-pointer">
            <ArrowLeft size={24} />
          </button>
          <div>
        ...
        <div className="flex gap-2">
          {isSoftware ? (
            <>
              <button onClick={() => openTxModal(selectedProject, 'ingreso')} className="px-4 py-2 bg-emerald-500/10 text-emerald-400 border border-emerald-500/30 rounded-xl text-sm font-bold hover:bg-emerald-500/20 transition-colors flex items-center gap-2 cursor-pointer"><Plus size={16}/> Ingreso</button>
              <button onClick={() => openTxModal(selectedProject, 'gasto')} className="px-4 py-2 bg-rose-500/10 text-rose-400 border border-rose-500/30 rounded-xl text-sm font-bold hover:bg-rose-500/20 transition-colors flex items-center gap-2 cursor-pointer"><Minus size={16}/> Gasto</button>
            </>
          ) : (
            <>
              <button onClick={() => openTxModal(selectedProject, 'venta')} className="px-4 py-2 bg-emerald-500/10 text-emerald-400 border border-emerald-500/30 rounded-xl text-sm font-bold hover:bg-emerald-500/20 transition-colors flex items-center gap-2 cursor-pointer"><Tag size={16}/> Registrar Venta</button>
              <button onClick={() => openTxModal(selectedProject, 'compra')} className="px-4 py-2 bg-rose-500/10 text-rose-400 border border-rose-500/30 rounded-xl text-sm font-bold hover:bg-rose-500/20 transition-colors flex items-center gap-2 cursor-pointer"><Package size={16}/> Comprar Stock</button>
            </>
          )}
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
        <div className="bg-[#1c1c1e] border border-[#2d2d2d] rounded-2xl p-4">
          <p className="text-xs font-bold text-gray-500 mb-1">{isSoftware ? 'Gastos Históricos' : 'Compras Totales'}</p>
          <p className="text-xl font-black text-rose-400">-{selectedProject.historicalInvested.toLocaleString()} €</p>
        </div>
        <div className="bg-[#1c1c1e] border border-[#2d2d2d] rounded-2xl p-4">
          <p className="text-xs font-bold text-gray-500 mb-1">{isSoftware ? 'Ingresos Totales' : 'Ventas Totales'}</p>
          <p className="text-xl font-black text-teal-400">+{selectedProject.historicalRevenue.toLocaleString()} €</p>
        </div>
        <div className="bg-[#1c1c1e] border border-[#2d2d2d] rounded-2xl p-4">
          <p className="text-xs font-bold text-gray-500 mb-1">Beneficio Neto</p>
          <p className={`text-xl font-black ${beneficioProj >= 0 ? 'text-emerald-400' : 'text-rose-400'}`}>{beneficioProj > 0 ? '+' : ''}{beneficioProj.toLocaleString()} €</p>
        </div>
        <div className="bg-[#1c1c1e] border border-[#2d2d2d] rounded-2xl p-4">
          <p className="text-xs font-bold text-gray-500 mb-1">{isSoftware ? 'ROI Histórico' : 'Stock Activo (Coste)'}</p>
          {isSoftware ? (
            <p className={`text-xl font-black ${roiProj >= 0 ? 'text-emerald-400' : 'text-rose-400'}`}>{roiProj > 0 ? '+' : ''}{roiProj.toFixed(1)}%</p>
          ) : (
            <p className="text-xl font-black text-amber-500">{selectedProject.activeInvested.toLocaleString()} €</p>
          )}
        </div>
      </div>

      {/* LIBRO DE CUENTAS (HISTORIAL) */}
        ...
        {transacciones.length > 0 ? (
          <div className="divide-y divide-[#2d2d2d]">
            {transacciones.map(tx => {
              ...
              return (
                <div key={tx.id} className="p-4 flex items-center justify-between hover:bg-[#1a1a1c] transition-colors">
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      ...
                      {tx.type === 'venta' && tx.cost !== undefined && (
                        <p className="text-xs text-gray-500 mt-0.5">Coste del artículo: {tx.cost.toLocaleString()} €</p>
                      )}
                    </div>
                  </div>
                  ...
                  <div className="text-right">
                    ...
                    {/* 🚀 EL CÁLCULO MÁGICO DE BENEFICIO POR VENTA */}
          ...
          <div className="p-10 text-center text-gray-500">
            <p className="font-medium text-sm">No hay movimientos en este proyecto.</p>
            <p className="text-xs mt-1">Registra tu primera compra, venta o gasto usando los botones superiores.</p>
          </div>
        )}
      </div>

      {/* MODAL DE TRANSACCIÓN INDIVIDUAL */}
            ...
            <div className="p-5 space-y-4">
              {(txModal.type === 'gasto' || txModal.type === 'compra') && (
                <div className="bg-rose-500/10 border border-rose-500/20 p-4 rounded-xl flex justify-between items-center mb-4">
                  <span className="text-xs font-bold text-rose-400">Se restará de tu banco:</span>
              ...
              <div>
                <label className="block text-xs font-bold text-gray-400 uppercase tracking-wider mb-1.5">
                  {txModal.type === 'venta' ? 'Precio al que lo has vendido (€)' : `Monto del ${txModal.type} (€)`}
                </label>
              ...
              {txModal.type === 'venta' && (
                <div className="mt-4">
                  <div className="flex justify-between items-end mb-1.5">
                    <label className="block text-xs font-bold text-gray-400 uppercase tracking-wider">¿Cuánto te costó comprarlo? (€)</label>
                  ...
                  {/* PREVISUALIZADOR DE BENEFICIO */}
                  {txAmount && txCost && (
                    <div className="bg-black/20 border border-[#2d2d2d] p-3 rounded-xl mt-3 flex justify-between">
                      <span className="text-xs font-bold text-gray-500">Beneficio limpio:</span>
                      <span className={`text-sm font-black ${(Number(txAmount) - Number(txCost)) >= 0 ? 'text-emerald-400' : 'text-rose-400'}`}>
                        {(Number(txAmount) - Number(txCost)) > 0 ? '+' : ''}{(Number(txAmount) - Number(txCost)).toFixed(2)} €
                      </span>
                    </div>
                  )}
                </div>
